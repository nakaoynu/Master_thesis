...
\section{各準位の緩和係数}
磁気感受率は式\eqref{eq:chi_linear}によって計算される. GGGは8準位あるので, 全\(8 \times 7 = 56\)個の遷移ペア( \(n \leftrightarrow n'\) )の寄与を考える必要がある. 計算コストの観点から, 本研究では, 式\eqref{eq:gamma_approximation}を仮定し, 表\ref{tab:gamma_transition}で示すように, 56個の遷移の緩和係数を7個の緩和係数で記述する. 以下, この仮定に基づく緩和係数を共有\(\gamma\)モデルと呼称する. 
\begin{equation}
...
各値での分布形状を図\ref{fig:studentt_distribution}に示す．
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{student_t.png}
    \caption{異なる自由度\(\nu\)におけるStudent-t分布の確率密度関数}
    \label{fig:studentt_distribution}
\end{figure}
