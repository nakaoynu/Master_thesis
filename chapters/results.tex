...
両形式ともに\(g_{J}\)因子はGd\(^{3+}\)の理論値（\(g \approx 2.0\)）に近い値を示した. 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{bayesian_results_scaled_loocv_20260119_063734/posterior_distributions_H.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{bayesian_results_scaled_loocv_20260119_063734/posterior_distributions_B.png}
...
緩和率$\gamma_k$は準位依存性を示し, 特に$k=1\text{--}3$の中間準位で大きな値（$\gamma \approx 0.14\text{--}0.17$~THz）を取る傾向が両モデルで見られる. Hモデルでは基底状態の緩和率$\gamma_0 = 0.023$~THzが特徴的に小さく, 高励起状態（$k \geq 4$）でも$\gamma \approx 0.01$~THzと低い. 一方, Bモデルでは$\gamma_2 = 0.010$~THzが異常に小さく, 他の準位とは異なる挙動を示す.
\clearpage
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{bayesian_results_scaled_loocv_20260119_063734/pair_plot_H.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{bayesian_results_scaled_loocv_20260119_063734/pair_plot_B.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{bayesian_results_scaled_loocv_20260119_063734/posterior_predictive_spectra_HB_comparison.png}
...
\ref{subsec:model_selection_criterion}節で述べたWAICおよびPSIS-LOOCVを用いて, HモデルとBモデルの予測性能を定量的に比較した. 表\ref{tab:waic_loo_results}に結果を示す.
...
\begin{itemize}
    ...
    \item 緩和率$\gamma_1$：Hモデル0.013~THz, Bモデル0.145~THz（約11倍の差）
\end{itemize}

これらのパラメータの差異は, 両モデルの数学的形式の違いを補償する方向に働いている. 特に, Bモデルでは$\mu_r^{B}$の発散を抑制するために緩和率が大きくなる傾向があり, 結果として観測される透過スペクトルは両モデルで類似する.
...
\begin{enumerate}
    ...
    \item \textbf{緩和率の準位依存性}：本研究では緩和率$\gamma_k$を準位ごとに独立なパラメータとして扱ったが, 物理的にはスピン-格子緩和やスピン-スピン緩和に基づく系統的な依存性が存在するはずである. より物理的に動機付けられた緩和率モデルの導入が望まれる.
