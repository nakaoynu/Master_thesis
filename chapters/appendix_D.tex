...
\section{磁気双極子相互作用}
\section*{ハミルトニアンの定義}
\(N\) 個の局在スピン \(\hat{\vb*{S}}_i\) と量子化磁場 \(\hat{\vb*{B}}\) との相互作用ハミルトニアン \(\hat{\mathcal{H}}_{\text{int}}\) はZeeman相互作用の形式（\(-\hat{\boldsymbol{\mu}} \cdot \hat{\vb*{B}}\)）で与えられる. 
\begin{equation}
    \hat{\mathcal{H}}_{\text{int}} = - \sum_{i=1}^N \hat{\boldsymbol{\mu}}_i \cdot \hat{\vb*{B}}
\end{equation}
磁気モーメント演算子は \(\hat{\boldsymbol{\mu}}_i = -g_J \mu_B \hat{\vb*{S}}_i\) である（\(g_J\): ランデのg因子, \(\mu_B\): ボーア磁子）. 式(\ref{eq:B_field})を代入すると, 磁場が \(x\) 成分のみを持つため, スピンの \(x\) 成分のみが結合する. 
\begin{equation}
    \hat{\mathcal{H}}_{\text{int}} = g_J \mu_B B_{\text{vac}} \sum_{i=1}^N \hat{S}_x^i (\hat{a} + \hat{a}^\dagger)
\end{equation}
...
\begin{align}
    \hat{\mathcal{H}}_{\text{int}} &= \frac{g_J \mu_B B_{\text{vac}}}{2} (\hat{J}_+ + \hat{J}_-) (\hat{a} + \hat{a}^\dagger) \\
    &= \frac{g_J \mu_B B_{\text{vac}}}{2} \left[ \underbrace{(\hat{a} \hat{J}_+ + \hat{a}^\dagger \hat{J}_-)}_{\text{回転項}} + \underbrace{(\hat{a} \hat{J}_- + \hat{a}^\dagger \hat{J}_+)}_{\text{反回転項}} \right]
\end{align}
...
単一スピン当たりの結合定数 \(\hbar g_0 = \frac{1}{2}g_J \mu_B B_{\text{vac}}\) を定義し, 集団増強された結合定数を \(g = g_0 \sqrt{N}\) とおくと, 全ハミルトニアンは以下のように求まる. 
\begin{equation}
    \hat{\mathcal{H}}_{\text{Dicke}} = \hbar \omega_c \hat{a}^\dagger \hat{a} + \hbar \omega_s \hat{J}_z + \frac{\hbar g}{\sqrt{N}} (\hat{a}^\dagger + \hat{a})(\hat{J}_+ + \hat{J}_-) \label{Dicke_Hamiltonian}
\end{equation}
...
\end{align}
これらをDickeハミルトニアン(式\eqref{Dicke_Hamiltonian})に代入し, 定数項を除くと, 以下の2次形式ボゾンハミルトニアン（Hopfieldハミルトニアン）が得られる. 
\begin{equation}
    \hat{\mathcal{H}}_{\text{Hop}} = \hbar \omega_c \hat{a}^\dagger \hat{a} + \hbar \omega_s \hat{b}^\dagger \hat{b} + \hbar g (\hat{a}^\dagger + \hat{a})(\hat{b}^\dagger + \hat{b}) 
    \label{eq:Hopfield}
\end{equation}
...
このハミルトニアン(式\eqref{eq:Hopfield})を対角化するために, 一般化されたBogoliubov変換（Hopfield変換）を導入し, 新たな固有演算子（ポラリトン演算子）\(\hat{p}_k\) (\(k=\text{LP, UP}\)) を定義する. 
\begin{equation}
...
\begin{equation}
    \Omega_{\pm}^2 = \frac{1}{2} \left[ \omega_c^2 + \omega_s^2 \pm \sqrt{(\omega_c^2 - \omega_s^2)^2 + 16 g^2 \omega_c \omega_s} \right]
\end{equation}
この結果は, USC領域において固有振動数が結合強度\(g\)に対して非線形に依存することを示している. 特に\(\Omega_-\)（Lower Polariton）は, Bloch-Siegertシフトを含んだエネルギー準位となる. 
\section{基底状態の量子性}
