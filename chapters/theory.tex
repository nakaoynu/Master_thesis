...
ここで, 式\eqref{H_total}の第1項\(\hat{\mathcal{H}}_{\text{CF}}\)は結晶場ハミルトニアンである. 結晶場ハミルトニアン \(\hat{\mathcal{H}}_{CF}\) は Stevens演算子 \(O_k^q\) を基底として展開され, その展開係数である結晶場パラメータ \(B_k^q\) が相互作用の強度と対称性を決定する. 
詳細は付録\ref{chap:CF_theory}にて記載する. 
\begin{equation}
\hat{\mathcal{H}}_{\text{CF}} = B_4(\hat{O}_4^0 + 5\hat{O}_4^4) + B_6(\hat{O}_6^0 - 21\hat{O}_6^4)
\label{eq:H_CF}
\end{equation}
式\eqref{H_total}の第2項\(\hat{\mathcal{H}}_{\text{Zeeman}}\)は, 外部静磁場\(\hat{\vb*{B}}_{\text{ext}}\)とスピン演算子\(\hat{\vb*{S}}\)の相互作用を表すゼーマン項である. 
\begin{equation}
\hat{\mathcal{H}}_{\text{Zeeman}} = g_{J} \mu_{B} \hat{\vb*{S}} \cdot \hat{\vb*{B}}_{\text{ext}}
\label{eq:H_Zeeman}
\end{equation}
% ここで, \(g_{J}\)はg因子, \(\mu_{B}\)はボーア磁子である. \\ 前述しているため省略可能
式\eqref{H_total}の第3項\(\hat{\mathcal{H}}_{H,B}^{'}\)は, 式\eqref{def:H_B}, 式\eqref{def:H_H}で定義した\(\text{Gd}^{3+}\)イオンの磁気モーメントと振動磁場とのZeeman相互作用を表す摂動項である. 
...
式\eqref{H_0}の無摂動ハミルトニアン\(\hat{\mathcal{H}}_{0}\)の固有エネルギー\(E_{n}\)と固有状態\(\ket{\psi_n}\)を用いて磁気感受率を計算すると, 以下の形式で与えられる. 
\begin{equation}
...
Kritzellらの実験では等方性物質であるGGGを用いて, Faraday配置（磁場 \(\vb*{B} \parallel \vb*{k} \parallel z\)）で透過スペクトルを測定している.\(^{\cite{Kritzell2024}}\) この配置では, 円偏光基底における磁気感受率テンソル\(\chi_{\pm}\)が重要となる. そこで, この直線偏光基底での磁気感受率テンソル(式\eqref{eq:chi_linear})を, 円偏光基底に変換する. 円偏光基底における磁気感受率テンソル\(\chi_{\pm}\)は, 直線偏光基底の成分\(\chi_{xx}, \chi_{yy}, \chi_{xy}, \chi_{yx}\)を用いて以下のように表される. (詳細は付録\ref{chap:appendix_circular}参照)
\begin{equation}
...
電磁気学では，一般に磁化\(\vb*{M}\)は磁場\(\vb*{H}\)に比例すると仮定される．本研究でも慣例に従い，式\eqref{eq:M_H_form}の形で磁化\(\vb*{M}\)を記述する．
\begin{equation}
...
\end{equation}
この関係式\eqref{def:relation_H_B_M}を用いると，式\eqref{def:susceptibility_LRT_EM}は以下のように書き換えられる.
\begin{align*}
...
まず, 各層における電磁波の伝搬を記述するために, 各層の特性インピーダンス\(Z\)と位相差\(\delta\)を定義する. 次に, 各層の境界での電磁波の連続条件を適用し, 層間の転送行列を構築する. 最終的に, 全体の転送行列を用いて, 入射波と透過波の関係を導出し, 透過スペクトルを計算する.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[>=Stealth, scale=1.0]
        ...
        % --- インピーダンス (中段) ---
        \node at (-\textOffsetSide, \h+\labelOffsetTwo) {$Z_1=1$};
        \node at (\slabW/2, \h+\labelOffsetTwo) {$Z_2=Z$};
        \node at (\slabW+\textOffsetSide, \h+\labelOffsetTwo) {$Z_3=1$};
        ...
        % 位相差表示    
        \draw[<->, dashed, darkgray] (0, 0) -- (\slabW, 0) node[midway, fill=cyan!10, align=center, inner sep=3pt] {Thickness $d$ \\ Phase $\delta=n k_0 d$};
    ...
    % --- キャプションとラベル ---
    \caption{転送行列法によるGGGの多層構造の計算モデル\\特性インピーダンス\(Z_1, Z_2, Z_3\)および厚さ\(d\)を持つGGGスラブを通過する電磁波の伝搬を解析するためのモデル図. 各界面での入射波、反射波、透過波、およびスラブ内部での多重反射波を示している. 下部には転送行列計算の概念図を示し, 入射波と透過波の関係を行列形式で表現していることを強調している.}
    \label{fig:ggg_tmm_model}
\end{figure}

\subsection{問題設定}
本解析では, 以下の3層構造を仮定し, 各領域の特性インピーダンス \(Z\) を定義する. 真空のインピーダンスで規格化を行い, \(Z_{vac} = 1\) とする.
\begin{itemize}
    \item \textbf{領域 1 (左側)}: 真空 (\(Z_1 = 1\))
    \item \textbf{領域 2 (薄膜)}: GGG (\(Z_2 = Z\), 厚さ \(d\))
    \item \textbf{領域 3 (右側)}: 真空 (\(Z_3 = Z_1 = 1\))
\end{itemize}

\subsection{転送行列の定式化 (Backward Definition)}
数学的な整合性を保つため, 標準的な光学テキスト（Born \& Wolf\(^{\cite{BornWolf1999}}\)）で採用されている\textbf{Backward転送行列}を使用する. これは, 出力側（右側）の場から入力側（左側）の場を逆算する形式である. 
\begin{equation}
...
各媒質の特性インピーダンスを \(Z_i, Z_j\) とする. 電磁気学的な境界条件に基づき, インピーダンスを用いたフレネル係数は以下のように定義される.
\begin{equation}
    r_{ij} = \frac{Z_j - Z_i}{Z_j + Z_i}, \quad t_{ij} = \frac{2Z_j}{Z_j + Z_i}
\end{equation}
本研究の系において, 界面1（真空 \(\to\) GGG）および界面2（GGG \(\to\) 真空）の係数は, 規格化インピーダンス \(Z\) を用いて次のように記述される.
\begin{align}
    \text{界面1:} & \quad r_{12} = \frac{Z-1}{Z+1}, \quad t_{12} = \frac{2Z}{Z+1} \\
    \text{界面2:} & \quad r_{21} = \frac{1-Z}{1+Z} = -r_{12}, \quad t_{21} = \frac{2}{1+Z}
\end{align}
これらを用いると, 界面行列 \(\mathbf{M}_{ij}\) は次式となる.
\begin{equation}
    \mathbf{M}_{ij} = \frac{1}{t_{ij}} \begin{pmatrix} 1 & r_{ij} \\ r_{ij} & 1 \end{pmatrix}
\end{equation}

\textbf{2. 伝搬行列（媒質 \(j\) 内部）:}
厚さ \(d\) の媒質内部での位相差 \(\delta\) を定義する. 
真空中の波数 \(k_0\) と屈折率 \(n\) を用いると, \(\delta\) は以下のように表される.
\begin{equation}
    \delta = n k_0 d
\end{equation}
ここで, GGGの比透磁率を \(\mu_r\), 比誘電率を \(\epsilon_r\) とすると, インピーダンスは \(Z = \sqrt{\mu_r / \epsilon_r}\), 屈折率は \(n = \sqrt{\mu_r \epsilon_r}\) である. したがって, 屈折率 \(n\) とインピーダンス \(Z\) は以下の関係にある.
\begin{equation}
    n = \frac{\mu_r}{Z}
\end{equation}
よって, 位相差\(\delta\)はインピーダンス\(Z\)を用いて次式で表される. 
\begin{equation}
  \delta = \frac{\mu_r k_0 d}{Z} 
\end{equation}
位置 \(z+d\) から \(z\) へ逆算する伝搬行列は以下の通りである. 
\begin{equation}
    \mathbf{M}_{prop} = \begin{pmatrix} e^{-i\delta} & 0 \\ 0 & e^{i\delta} \end{pmatrix}
\end{equation}
（注：Backward行列のため, 進行波に対して逆位相 \(e^{-i\delta}\) を乗じている.）
...
\end{equation}
透過係数の導出に必要な \((1,1)\) 成分のみを計算すると, 以下のようになる.
\begin{align}
    (\mathbf{M}_{GGG})_{11} &= \frac{1}{t_{12}t_{21}} \left( e^{-i\delta} + r_{12} r_{21} e^{i\delta} \right)
\end{align}
ここで, 定義したインピーダンス表現を代入して整理する. 
\begin{align}
    \frac{1}{t_{12}t_{21}} &= \frac{(1+Z)^2}{4Z}, \quad r_{12} r_{21} = - \left( \frac{1-Z}{1+Z} \right)^2
\end{align}
したがって, 
\begin{equation}
    (\mathbf{M}_{GGG})_{11} = \frac{(1+Z)^2}{4Z} e^{-i\delta} - \frac{(1-Z)^2}{4Z} e^{i\delta}
\end{equation}

\subsection{透過係数の導出と結論}
スラブ全体の透過係数 \(t_{slab}\) は \(\frac{1}{(\mathbf{M}_{GGG})_{11}}\) で求まる.
\begin{align}
    t_{slab} &= \left[ \frac{(1+Z)^2}{4Z} e^{-i\delta} - \frac{(1-Z)^2}{4Z} e^{i\delta} \right]^{-1} \\
    &= \frac{4Z}{(1+Z)^2 e^{-i\delta} - (1-Z)^2 e^{i\delta}}
\end{align}
標準的な形式に合わせるため, 分母・分子に \(e^{i\delta}\) を乗じて整理する.
\begin{equation}
    t_{slab} = \frac{4Z e^{i\delta}}{(1+Z)^2 - (1-Z)^2 e^{2i\delta}}
\end{equation}
以上により, インピーダンス \(Z\) をパラメータとした透過係数の表式が導出された. この結果は, GGG試料の透過スペクトルを計算する際に用いられる.
...
\subsubsection*{事後分布}
事後分布 \(p(\theta|D)\) は, 式\eqref{equation_bayesian}にあるように, 観測データに基づいて更新されたパラメータの確率分布を表す. 事後分布は, 事前分布と尤度関数の積に比例し, 正規化定数 \(p(D)\) によって正規化される. 
...
ベイズ推定において, 事後分布 \(P(\theta|D)\) を直接計算することは困難であるため, マルコフ連鎖モンテカルロ法（Markov Chain Monte Carlo: MCMC）が広く用いられる. MCMCは, 事後分布から, その分布に従う乱数（サンプル）を効率的に生成するためのアルゴリズム群である.
