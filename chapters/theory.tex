...
ベイズ推定の最大の目的は, 得られた事後分布 \(p(\vb*{\theta} | D)\) から, パラメータの期待値および 95\% 最高密度区間を算出することである. 事後分布の広がりが鋭いほど推定精度が高く, 分布が多峰性を示す場合は, モデル内のパラメータ間に強い相関が存在するか, あるいは物理モデルが現象を記述するのに不十分である可能性を示唆する.\\
