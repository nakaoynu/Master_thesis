...
\title{ファラデー配置における磁気感受率テンソルの円偏光基底への変換}
\author{数理導出ノート}
\date{\today}
...
\section{問題設定}
ファラデー配置（磁場 \(\vb*{B} \parallel \vb*{k} \parallel z\)）における, 等方性媒質（または立方晶系）の線形応答を考える. 
...
\end{equation}
これは, ファラデー配置において円偏光がこの系の\textbf{固有モード (Eigenmodes)} であることを意味する. 
右円偏光 ($\sigma_+$) と左円偏光 ($\sigma_-$) は互いに独立に伝搬し, それぞれの複素屈折率 $n_{\pm} \approx \sqrt{1+\chi_{\pm}}$ を感じる. この屈折率差 $\Delta n = n_+ - n_-$ がファラデー回転の起源となる. 
