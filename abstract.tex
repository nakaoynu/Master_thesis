...
近年のナノテクノロジーや超伝導回路技術の進展により，結合定数\(g\)が原子や共振器の共振周波数\(\omega\)の数10\%程度に達する「超強結合（Ultra-Strong Coupling: USC）領域」，さらには\(g \geq \omega\)となる「深強結合（Deep Strong Coupling: DSC）領域」へのアクセスが可能になった．
これらの領域では，従来の摂動論的アプローチや回転波近似（RWA）が破綻し, 光と物質の境界が曖昧な混成状態の準粒子（ポラリトン）が形成される$^{\cite{Ciuti2005, FornDiaz2019, Kockum2019}}$．

USC領域において注目される現象として，1973年にHeppらが提唱した「超放射相転移（SRPT）」が挙げられる．$^{\cite{Hepp1973, Wang1973}}$多数の原子と単一の光モードが相互作用するDickeモデル$^{\cite{Dicke1954}}$において，結合強度が臨界点を超えた瞬間に，基底状態が巨視的な光子数を伴う状態へと相転移する現象である．これに伴い，光子場と物質場がそれぞれ静的横方向電磁場と静的分極として熱平衡状態で自発的に現れる．SRPTの特異的な点は，光と物質の結合系の熱平衡状態に関連する物理現象であることだ．
...
坂田らは, 線形応答理論を用いてGGGの磁気感受率 \(\chi (\omega)\) を計算し, 両形式に基づく透過スペクトルを算出したが, 次の2点の理由からどちらの形式が実験データを説明するか結論付けるには至らなかった．
Kritzellらの実験は, GGGのTHz磁気光学応答を磁場依存性・温度依存性の2つの観点から行っていたが, 坂田らの解析では, それぞれの形式に基づく理論透過スペクトルと実験データの比較が, 磁場依存性・温度依存性のいずれか一方に限られていた点．
また, 両形式に基づく理論透過スペクトルと実験データの比較が定性的な比較にとどまっていた. \(^{\cite{Sakata2025}}\)
